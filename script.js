...
function addFireCountyLabels(counties) {
  ...
  counties.features?.forEach((feature) => {
    ...
    const icon = L.divIcon({
      className: "county-label fire-county-label",
      html: `<span>${name}</span>`,
      iconSize: [58, 22],
      iconAnchor: [29, 11]
    });
...
function syncModeLayers(mode) {
  ...
  const weatherLabelLayers = [daneCityVillageLabelLayer, daneTownshipLabelLayer, regionalLabelLayer, extendedCountyLabelLayer, extendedCityLabelLayer];
  ...
  if (fireCountyLayer) {
    if (mode === "fire") {
      ...
      map.fitBounds(fireCountyLayer.getBounds(), { padding: [16, 16] });
    } else {
...
async function loadRegionalBoundaries() {
  try {
    const [counties, towns, places, extendedCounties, fireCounties] = await Promise.all([
      ...
      fetchJson(tigerQueryUrl(TIGER_COUNTIES, null, FIRE_WHERE))
    ]);
