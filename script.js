...
const COUNTY_CODES = ["025", "021", "111", "049", "045", "105", "055", "027", "065"];
...
function addFeatureLabel(feature, type, seen) {
  ...
  L.marker(bounds.getCenter(), {
    ...
    interactive: false,
    zIndexOffset: 900
  }).addTo(county === "025" ? daneLabelLayer : regionalLabelLayer);
}
...
async function loadRegionalBoundaries() {
  try {
    ...
    const countyBorders = L.geoJSON(counties, {
      ...
      style: (feature) => ({
        color: "#ffffff",
        weight: feature.properties?.COUNTY === "025" ? 9 : 7,
        opacity: 0.92,
        fill: false
      }),
    ...
    const daneOutline = L.geoJSON(counties, {
      ...
      style: (feature) => ({
        color: "#d93025",
        weight: feature.properties?.COUNTY === "025" ? 4.8 : 3.2,
        opacity: 1,
        fill: false
      })
    }).addTo(map);
