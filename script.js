...
let currentForecastUrl = null;
let latestObservationText = "";
let latestForecastText = "";
let activeDaneAlerts = [];
...
const FEATURE_COPY = {
  ...
  forecast: {
    ...
    summary: "Simple NWS forecast for the City-County Bldg location, focused on today, tomorrow, and the coming days.",
    details: [
      ["Location", "City-County Bldg"],
      ["Source", "NWS Forecast"],
...
map.getPane("lightningPane").style.zIndex = 440;
...
async function currentForecast() {
  if (currentForecastUrl) return currentForecastUrl;
  ...
  currentForecastUrl = point.properties?.forecast;
  if (!currentForecastUrl) throw new Error("No forecast found");
  return currentForecastUrl;
}
...
function syncModeLayers(mode) {
  ...
  const showWeather = mode === "weather" || mode === "forecast";
  document.body.classList.toggle("fire-mode", mode === "fire");
  document.body.classList.toggle("forecast-mode", mode === "forecast");
...
function lightningMention(text) {
  return /\b(lightning|thunder|thunderstorm|t-?storm)\b/i.test(text || "");
}

function alertFeatureCenter(feature) {
  if (feature?.geometry) {
    const bounds = L.geoJSON(feature).getBounds();
    if (bounds.isValid()) return bounds.getCenter();
  }
  return L.latLng(CURRENT_CONDITIONS_LOCATION.lat, CURRENT_CONDITIONS_LOCATION.lng);
}

function addLightningMarker(latLng, label) {
  const icon = L.divIcon({
    className: "lightning-marker",
    html: `<strong>Lightning Possible</strong><span>${escapeHtml(label)}</span>`,
    iconSize: [172, 50],
    iconAnchor: [86, 25]
  });
  L.marker(latLng, {
...
function syncLightningLayer() {
  lightningLayer.clearLayers();
  if (!$("toggleLightningLayer")?.checked || !["weather", "forecast"].includes($("featureSelect").value)) return;
  const alertTextCombined = activeDaneAlerts.map(alertText).join(" ");
  let added = false;
  activeDaneAlerts.forEach((feature) => {
    if (!lightningMention(alertText(feature))) return;
    addLightningMarker(alertFeatureCenter(feature), "Active alert mentions thunder/lightning");
    added = true;
  });
  if (!added && (lightningMention(latestObservationText) || lightningMention(alertTextCombined) || lightningMention(latestForecastText))) {
    addLightningMarker(L.latLng(CURRENT_CONDITIONS_LOCATION.lat, CURRENT_CONDITIONS_LOCATION.lng), "NWS wording mentions thunder/lightning near Dane County");
  }
}

function forecastCard(period) {
  return `<article class="forecast-card">
    <div><strong>${escapeHtml(period.name)}</strong><span>${escapeHtml(period.shortForecast || "Forecast")}</span></div>
    <b>${period.temperature} ${escapeHtml(period.temperatureUnit || "F")}</b>
...
async function loadForecast() {
  try {
    const forecastUrl = await currentForecast();
    const forecast = await fetchJson(forecastUrl);
    const periods = (forecast.properties?.periods || []).slice(0, 14);
    latestForecastText = periods.map((period) => `${period.shortForecast || ""} ${period.detailedForecast || ""}`).join(" ");
    syncLightningLayer();
    $("forecastList").innerHTML = periods.length
      ? periods.map(forecastCard).join("")
      : "Forecast temporarily unavailable.";
  } catch (error) {
    latestForecastText = "";
    syncLightningLayer();
    $("forecastList").textContent = "Could not reach the NWS forecast feed.";
  }
}

function openAlertDrawer(feature) {
  if (!feature) return;
  const p = feature.properties || {};
  const until = p.ends ? new Date(p.ends).toLocaleString("en-US", { timeZone: "America/Chicago" }) : "Until further notice";
  $("alertDrawerTitle").textContent = p.event || "Alert Details";
  $("alertDrawerMeta").innerHTML = [
    ["Severity", p.severity || "--"],
    ["Expires", until],
    ["Areas", p.areaDesc || "Dane County"]
  ].map(([term, value]) => `<div><dt>${term}</dt><dd>${escapeHtml(value)}</dd></div>`).join("");
  $("alertDrawerText").textContent = p.description || p.headline || "No additional alert text was provided.";
  $("alertDrawerSafety").textContent = p.instruction || "Follow local emergency guidance and monitor official NWS updates.";
...
async function loadWeather() {
  try {
    ...
    latestObservationText = `${p.textDescription || ""} ${p.rawMessage || ""}`;
    syncLightningLayer();
...
async function loadAlerts() {
  try {
    ...
    $("alertsList").innerHTML = daneAlerts.slice(0, 4).map((feature, index) => {
      const p = feature.properties;
      const until = p.ends ? new Date(p.ends).toLocaleTimeString("en-US", { timeZone: "America/Chicago" }) : "until further notice";
      return `<button class="alert-item" type="button" data-alert-index="${index}"><strong>${p.event}</strong><small>${p.severity || "Alert"} - ${until}</small></button>`;
    }).join("");
  } catch (error) {
    activeDaneAlerts = [];
    syncLightningLayer();
...
function refreshRadar() {
  if (!["weather", "forecast"].includes($("featureSelect").value)) return;
...
function refreshActiveMap() {
  if (["weather", "forecast"].includes($("featureSelect").value)) {
    refreshRadar();
    loadAlerts();
    loadForecast();
  } else {
...
$("alertsList").addEventListener("click", (event) => {
  ...
  if (!button) return;
  openAlertDrawer(activeDaneAlerts[Number(button.dataset.alertIndex)]);
});
$("closeAlertDrawer").addEventListener("click", closeAlertDrawer);
$("toggleLightningLayer").addEventListener("change", syncLightningLayer);
$("mapThemeSelect").addEventListener("change", (event) => {
...
setupPasswordGate();
updateClock();
setMapTheme($("mapThemeSelect").value);
