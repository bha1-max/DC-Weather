...
const REGIONAL_BOUNDS = [[42.45, -90.45], [43.75, -88.45]];
const USER_AGENT_NOTE = "Dane County Live Weather demo";
const COUNTY_CODES = ["025", "021", "111", "049", "045", "105", "055", "027"];
...
function tigerQueryUrl(baseUrl, layerId, where) {
  ...
  url.searchParams.set("outFields", "NAME,STATE,COUNTY");
...
async function loadRegionalBoundaries() {
  try {
    const countyWhere = `STATE='55' AND COUNTY IN (${COUNTY_QUERY})`;
    const [counties, towns, places] = await Promise.all([
      fetchJson(tigerQueryUrl(TIGER_COUNTIES, null, countyWhere)),
      fetchJson(tigerQueryUrl(TIGER_PLACES, 1, countyWhere)),
      fetchJson(tigerQueryUrl(TIGER_PLACES, 4, countyWhere))
    ]);
    daneBoundaryLayers.forEach((layer) => map.removeLayer(layer));

    const countyBorders = L.geoJSON(counties, {
      pane: "boundaryPane",
      style: (feature) => ({
        color: feature.properties?.COUNTY === "025" ? "#ffffff" : "#4b5563",
        weight: feature.properties?.COUNTY === "025" ? 9 : 3,
        opacity: feature.properties?.COUNTY === "025" ? 0.95 : 0.9,
        fill: false
      }),
    ...
    const daneOutline = L.geoJSON(counties, {
      filter: (feature) => feature.properties?.COUNTY === "025",
      pane: "boundaryPane",
      style: {
        color: "#d93025",
        weight: 4.5,
    ...
    daneOutline.bindTooltip("Dane County", { permanent: false });
    daneBoundaryLayers = [countyBorders, townOutlines, placeOutlines, daneOutline];
    map.fitBounds(countyBorders.getBounds(), { padding: [14, 14] });
  } catch (error) {
