...
const CURRENT_CONDITIONS_LOCATION = {
  name: "City-County Bldg",
  address: "210 Martin Luther King Jr Blvd, Madison, WI",
  lat: 43.0731,
  lng: -89.3842
};
let currentConditionsStationUrl = null;
let currentForecastHourlyUrl = null;
...
const FEATURE_COPY = {
  weather: {
    title: "Data Notes",
    summary: "Current conditions are centered on the City-County Bldg in Madison, using the nearest NWS observation feed for that exact location.",
    details: [
      ["Location", "City-County Bldg"],
      ["Radar", "NOAA/NWS MRMS"],
      ["Current Conditions", "Every 60 seconds"],
      ["Radar/Alerts", "Every 60 seconds"]
    ]
  },
  hourly: {
    title: "Hour-to-Hour Notes",
    summary: "Shows recent observations, current conditions, and the NWS hourly forecast for the City-County Bldg location.",
    details: [
      ["Location", "City-County Bldg"],
      ["Recent", "Previous 4 hours"],
...
const DANE_CITY_VILLAGE_NAMES = new Set(PLACES.map(([name]) => name.toLowerCase()));
const FELLOW_PLACES = [
...
let selectedHourlyMapLayer = L.layerGroup().addTo(map);
...
function addManualRegionalLabels(seen = new Set()) {
...
async function currentConditionsStation() {
  if (currentConditionsStationUrl) return currentConditionsStationUrl;
  const pointUrl = `https://api.weather.gov/points/${CURRENT_CONDITIONS_LOCATION.lat},${CURRENT_CONDITIONS_LOCATION.lng}/stations`;
  const stations = await fetchJson(pointUrl);
  currentConditionsStationUrl = stations.features?.[0]?.id;
  if (!currentConditionsStationUrl) throw new Error("No nearby observation station found");
  return currentConditionsStationUrl;
}

async function currentHourlyForecastUrl() {
  if (currentForecastHourlyUrl) return currentForecastHourlyUrl;
  const pointUrl = `https://api.weather.gov/points/${CURRENT_CONDITIONS_LOCATION.lat},${CURRENT_CONDITIONS_LOCATION.lng}`;
  const point = await fetchJson(pointUrl);
  currentForecastHourlyUrl = point.properties?.forecastHourly;
  if (!currentForecastHourlyUrl) throw new Error("No hourly forecast found");
  return currentForecastHourlyUrl;
}
...
function syncModeLayers(mode) {
  ...
  const showWeather = mode === "weather" || mode === "hourly";
  document.body.classList.toggle("fire-mode", mode === "fire");
  document.body.classList.toggle("hourly-mode", mode === "hourly");
  ...
  if (mode === "hourly") {
    selectedHourlyMapLayer.addTo(map);
  } else {
    selectedHourlyMapLayer.clearLayers();
    map.removeLayer(selectedHourlyMapLayer);
  }
...
function addFeatureLabel(feature, type, seen) {
  ...
  if (isDaneFeature(feature) && type === "town" && DANE_CITY_VILLAGE_NAMES.has(labelKey(name))) return;
  ...
  const key = targetLayer === regionalLabelLayer
    ? labelKey(name)
    : `${feature.properties?.STATE}:${feature.properties?.COUNTY}:${type}:${labelKey(name)}`;
  if (seen.has(key)) return;
  seen.add(key);
...
async function loadRegionalBoundaries() {
  try {
    ...
    addManualRegionalLabels(seenLabels);
    syncLabelToggles();
...
function setFeatureMode(mode) {
  ...
  syncModeLayers(mode);
  if (mode === "hourly") loadHourToHourWeather();
}

function alertText(feature) {
  ...
  return [
    ...
    p.description,
    p.instruction,
    p.parameters?.NWSheadline?.join(" ")
  ].filter(Boolean).join(" ");
}

function updateWeatherAlertBorder(alerts) {
  document.body.classList.remove("thunder-alert", "tornado-alert", "tornado-touchdown-alert");
  const combined = alerts.map(alertText).join(" ");
  if (/\btornado\b/i.test(combined)) {
    document.body.classList.add(/\b(touchdown|touched down|on the ground|confirmed tornado)\b/i.test(combined)
      ? "tornado-touchdown-alert"
      : "tornado-alert");
    return;
  }
  if (/\b(thunderstorm|severe thunderstorm)\b/i.test(combined)) {
    document.body.classList.add("thunder-alert");
  }
}
...
function observationRow(properties) {
  const temp = fmt(cToF(properties.temperature?.value), " F");
  const wind = mpsToMph(properties.windSpeed?.value);
  const windText = wind === null ? "--" : `${compass(properties.windDirection?.value)} ${wind} mph`;
  return `<article class="hour-row" data-hour-kind="observation" data-hour-time="${escapeHtml(properties.timestamp)}" data-hour-temp="${escapeHtml(temp)}" data-hour-summary="${escapeHtml(properties.textDescription || "Observation")}" data-hour-wind="${escapeHtml(windText)}">
    <time>${hourLabel(properties.timestamp)}</time>
    <strong>${temp}</strong>
    <span>${escapeHtml(properties.textDescription || "Observation")}</span>
    <small>${windText}</small>
  </article>`;
}

function showHourOnMap({ time, temp, summary, wind, kind }) {
  selectedHourlyMapLayer.clearLayers();
  if ($("featureSelect").value !== "hourly") return;
  const title = kind === "forecast" ? "Forecast" : "Observed";
  const icon = L.divIcon({
    className: "hour-map-marker",
    html: `<strong>${escapeHtml(temp)}</strong><span>${escapeHtml(summary)}</span>`,
    iconSize: [150, 54],
    iconAnchor: [75, 54]
  });
  L.marker([CURRENT_CONDITIONS_LOCATION.lat, CURRENT_CONDITIONS_LOCATION.lng], {
    icon,
    zIndexOffset: 1100
  }).addTo(selectedHourlyMapLayer).bindPopup(
    `<strong>${title} ${hourLabel(time)}</strong><br>${escapeHtml(summary)}<br>${escapeHtml(temp)}${wind ? `<br>${escapeHtml(wind)}` : ""}`,
    { closeButton: false }
  ).openPopup();
  map.setView([CURRENT_CONDITIONS_LOCATION.lat, CURRENT_CONDITIONS_LOCATION.lng], Math.max(map.getZoom(), 11), { animate: true });
}

function forecastRow(period) {
  return `<article class="hour-row" data-hour-kind="forecast" data-hour-time="${escapeHtml(period.startTime)}" data-hour-temp="${period.temperature} ${escapeHtml(period.temperatureUnit || "F")}" data-hour-summary="${escapeHtml(period.shortForecast || "Forecast")}" data-hour-wind="${escapeHtml(`${period.windDirection || ""} ${period.windSpeed || ""}`.trim())}">
    <time>${hourLabel(period.startTime)}</time>
    <strong>${period.temperature} ${escapeHtml(period.temperatureUnit || "F")}</strong>
    <span>${escapeHtml(period.shortForecast || "Forecast")}</span>
    <small>${escapeHtml(period.windDirection || "")} ${escapeHtml(period.windSpeed || "")}</small>
  </article>`;
}

function renderForecastDayTabs(periods) {
  const days = [];
  const seen = new Set();
  periods.forEach((period) => {
    ...
    if (seen.has(key)) return;
    seen.add(key);
    days.push({ key, label: dayLabel(period.startTime) });
  });
  if (!selectedForecastDate || !seen.has(selectedForecastDate)) selectedForecastDate = days[0]?.key || null;
  $("forecastDayTabs").innerHTML = days.slice(0, 7).map((day) => (
    `<button type="button" class="${day.key === selectedForecastDate ? "active" : ""}" data-forecast-day="${day.key}">${day.label}</button>`
  )).join("");
}

function renderFutureHours(periods) {
  const selected = periods.filter((period) => dayKey(period.startTime) === selectedForecastDate);
  $("futureHours").innerHTML = selected.length
    ? selected.map(forecastRow).join("")
    : "No hourly forecast available for this day.";
}

function selectHourRow(row) {
  document.querySelectorAll(".hour-row.selected").forEach((item) => item.classList.remove("selected"));
  row.classList.add("selected");
  showHourOnMap({
    time: row.dataset.hourTime,
    temp: row.dataset.hourTemp,
    summary: row.dataset.hourSummary,
    wind: row.dataset.hourWind,
    kind: row.dataset.hourKind
  });
}

async function loadHourToHourWeather() {
  if ($("featureSelect").value !== "hourly") return;
  try {
    const stationUrl = await currentConditionsStation();
    const forecastUrl = await currentHourlyForecastUrl();
    const now = new Date();
    const start = new Date(now.getTime() - (4 * 60 * 60 * 1000));
    const [observations, forecast] = await Promise.all([
      fetchJson(`${stationUrl}/observations?start=${start.toISOString()}&end=${now.toISOString()}`),
      fetchJson(forecastUrl)
    ]);
    const obs = (observations.features || [])
      .map((feature) => feature.properties)
      .filter((props) => props?.timestamp)
      .sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp));
    const latest = obs[obs.length - 1];
    $("previousHours").innerHTML = obs.length
      ? obs.slice(-4).map(observationRow).join("")
      : "No recent observations available.";
    $("currentHour").innerHTML = latest
      ? observationRow(latest)
      : "Current observation unavailable.";
    const periods = (forecast.properties?.periods || []).slice(0, 24 * 7);
    renderForecastDayTabs(periods);
    renderFutureHours(periods);
    const firstFutureRow = $("futureHours").querySelector(".hour-row");
    if (firstFutureRow) selectHourRow(firstFutureRow);
  } catch (error) {
    $("previousHours").textContent = "Could not reach recent observations.";
    $("currentHour").textContent = "Current observation unavailable.";
    $("futureHours").textContent = "Could not reach the hourly forecast.";
  }
}
...
async function loadWeather() {
  try {
    ...
    $("summary").textContent = p.textDescription || `Latest ${stationId} observation`;
    ...
    $("obsAge").textContent = when
      ? `City-County Bldg nearest station ${stationId}. Observed ${when.toLocaleString("en-US", { timeZone: "America/Chicago" })}`
      : "Observation time unavailable";
    if ($("featureSelect").value === "weather") {
      $("featureDetails").innerHTML = [
        ["Location", CURRENT_CONDITIONS_LOCATION.name],
        ["Address", CURRENT_CONDITIONS_LOCATION.address],
...
async function loadAlerts() {
  try {
    ...
    if (!daneAlerts.length) {
      updateWeatherAlertBorder([]);
      $("alertsList").textContent = "No active Dane County alerts found.";
      return;
    }
    updateWeatherAlertBorder(daneAlerts);
...
function refreshRadar() {
  if (!["weather", "hourly"].includes($("featureSelect").value)) return;
...
function refreshActiveMap() {
  if ($("featureSelect").value === "weather" || $("featureSelect").value === "hourly") {
    refreshRadar();
    loadAlerts();
  } else {
    refreshFireRisk();
...
function refreshCurrentConditions() {
  if ($("featureSelect").value === "weather") {
    loadWeather();
  } else if ($("featureSelect").value === "hourly") {
    loadHourToHourWeather();
  }
}
...
$("refreshHourly").addEventListener("click", loadHourToHourWeather);
$("forecastDayTabs").addEventListener("click", (event) => {
  const button = event.target.closest("[data-forecast-day]");
  if (!button) return;
  selectedForecastDate = button.dataset.forecastDay;
  loadHourToHourWeather();
});
$("previousHours").addEventListener("click", (event) => {
  const row = event.target.closest(".hour-row");
  if (row) selectHourRow(row);
});
$("currentHour").addEventListener("click", (event) => {
  const row = event.target.closest(".hour-row");
  if (row) selectHourRow(row);
});
$("futureHours").addEventListener("click", (event) => {
  const row = event.target.closest(".hour-row");
  if (row) selectHourRow(row);
});
...
setInterval(refreshActiveMap, FAST_REFRESH_MS);
setInterval(refreshCurrentConditions, WEATHER_REFRESH_MS);
